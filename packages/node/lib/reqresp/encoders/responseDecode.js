...
import { ContextBytesType, CONTEXT_BYTES_FORK_DIGEST_LENGTH, } from "../types.js";
 ...
 * ```
 */
export function responseDecode(protocol, cbs) {
    return async function* responseDecodeSink(source) {
        ...
        while (!bufferedSource.isDone) {
            ...
            const type = protocol.responseType();
 ...
 */
export async function readResultHeader(bufferedSource) {
 ...
 * ```
 */
export async function readErrorMessage(bufferedSource) {
 ...
 */
export async function readContextBytesForkDigest(bufferedSource) {
    for await (const buffer of bufferedSource) {
