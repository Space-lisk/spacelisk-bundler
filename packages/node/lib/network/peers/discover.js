...
export class PeerDiscovery {
    constructor(modules, opts) {
         ...
         */
        this.onDiscoveredPeer = async (evt) => {
         ...
         */
        this.onDiscoveredENR = async (enr) => {
        ...
        if (numBootEnrs === 0) {
            ...
            this.logger.debug({ bootEnrs: numBootEnrs }, "PeerDiscovery: number of bootEnrs");
        }
    }
    async start() {
     ...
     */
    async handleDiscoveredPeer(peerId, multiaddrTCP, chainId) {
        try {
    ...
    async dialPeer(cachedPeer) {
        ...
        try {
            ...
            this.logger.debug({ peer: peerIdShort, error: e }, "Error dialing discovered peer");
        }
    }
