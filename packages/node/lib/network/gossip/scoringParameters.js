 ...
 */
export function computeGossipPeerScoreParams({ config, }) {
    ...
    const targetValue = decayConvergence(behaviourPenaltyDecay, 10 / SLOTS_PER_EPOCH) -
...
function getAllTopicsScoreParams(precomputedParams) {
    const { epochDurationMs } = precomputedParams;
    const topicsParams = {};
    // other topics
    topicsParams[stringifyGossipTopic({
...
function getTopicScoreParams({ epochDurationMs, slotDurationMs, scoreParameterDecayFn }, { topicWeight, expectedMessageRate, firstMessageDecayTime, meshMessageInfo, }) {
