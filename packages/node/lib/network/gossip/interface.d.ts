...
export type GossipFnByType = {
    [GossipType.user_operations]: (userOpsWithEP: ts.VerifiedUserOperation) => Promise<void> | void;
};
 ...
 */
export type GossipValidatorFn = (topic: GossipTopic, msg: Message, propagationSource: PeerIdStr, seenTimestampSec: number) => Promise<TopicValidatorResult>;
export type ValidatorFnsByType = {
    [K in GossipType]: GossipValidatorFn;
};
export type GossipHandlerFn = (object: GossipTypeMap[GossipType], topic: GossipTopicMap[GossipType], peerIdStr: string, seenTimestampSec: number, gossipSerializedData: Uint8Array) => Promise<void>;
export type GossipHandlers = {
    [K in GossipType]: (object: GossipTypeMap[K], topic: GossipTopicMap[K], peerIdStr: string, seenTimestampSec: number, gossipSerializedData: Uint8Array) => Promise<void>;
};
export type ResolvedType<F extends (...args: any) => Promise<any>> = F extends (...args: any) => Promise<infer T> ? T : never;
